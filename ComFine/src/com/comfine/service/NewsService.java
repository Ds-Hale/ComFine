...
public ModelAndView news_control(ModelMap model) {
	int count = News.getCount();	
	List<News> news  = nJdbc.getAllNews();
	model.put("news",news);
	model.put("currentpage", 0);
	model.put("end", (count/5)/5);
	return new ModelAndView("/news/newshoutai","message",model);
}
...
public ModelAndView newspage(int page,ModelMap model) {
	int count = News.getCount()/5;	
	List <News> newslist = nJdbc.getNewsRange(page, page+5);
	if(newslist.isEmpty() && page>=count){model.put("currentpage",count+1);
	}
	else{
	model.put("currentpage", page);}
	model.put("news", newslist);
	model.put("end", count+1);
	return new ModelAndView("/news/newshoutai","message",model);
}
